...
struct ShapeTreeHandler: APIProtocol, Sendable {
  ...
  func runDailySummary(
    ...
    guard let service = dailySummaryService else {
      return .internalServerError(.init(body: .json(Self.errorBody("Daily summary service not configured."))))
    }
  ...
  func getDailySummary(
    ...
    // Load entry count from the workflow cache if available
    let output = DailySummaryOutput(dayKey: dayKey, summary: content, entryCount: 0)
    return .ok(
      .init(
        body: .json(
          .init(
