...
final class SMTPRequestEncoder: MessageToByteEncoder, Sendable {
  ...
  func encode(data: SMTPRequest, out: inout ByteBuffer) throws {
    ...
    case .transferData(let email):
      let date = Date()
      let dateFormatted = SMTPMessageEncoding.rfc5322DateString(from: date)
      let subject = SMTPMessageEncoding.sanitizeHeaderValue(email.subject)
      ...
      out.writeString(SMTPMessageEncoding.dotStuffedBody(email.body))
      out.writeString("\r\n.")
    case .quit:
  ...
  private func formatMIME(emailAddress: String, name: String?) -> String {
    let address = SMTPMessageEncoding.sanitizeHeaderValue(emailAddress)
    if let name {
      let sanitizedName = SMTPMessageEncoding.sanitizeHeaderValue(name)
      return "\(sanitizedName) <\(address)>"
    }
    return address
  }
}
